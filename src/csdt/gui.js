...
SpriteMorph.prototype.freshPalette = function (category) {
  ...
  palette.userMenu = function () {
    ...
    function hasHiddenPrimitives() {
      ...
      return Object.keys(hiddens).some(
        (any) =>
          !isNil(defs[any]) &&
    ...
    function canHidePrimitives() {
      return palette.contents.children.some((any) =>
        contains(Object.keys(SpriteMorph.prototype.blocks), any.selector)
      );
    }
    ...
    if (canHidePrimitives()) {
      menu.addItem("hide primitives", function () {
        ...
        (more[category] || []).forEach(
          (sel) => (StageMorph.prototype.hiddenPrimitives[sel] = true)
        );
...
IDE_Morph.prototype.fixLayout = function (situation) {
  ...
  if (situation !== "refreshPalette") {
    ...
    if (this.isEmbedMode) {
      ...
      this.stage.setScale(
        this.isSmallStage ? this.stageRatio : this.initialScaleSize
      );
    ...
    if (StageMorph.prototype.basicLayout)
      this.spriteBar.setLeft(this.paletteWidth + padding - 200);
...
IDE_Morph.prototype.popupMediaImportDialog = function (folderName, items) {
  ...
  dialog.cancel = function () {
    ...
    let audioArray = frame.children[0].children.filter(
      (a) => a instanceof SoundIconMorph
    );
...
SnapSerializer.prototype.rawLoadProjectModel = function (xmlNode, remixID) {
  ...
  if (model.stage.attributes.width) {
    StageMorph.prototype.dimensions.x = Math.max(
      +model.stage.attributes.width,
      240
    );
  }
  if (model.stage.attributes.height) {
    StageMorph.prototype.dimensions.y = Math.max(
  ...
  this.project.stage.children.forEach((sprite) => {
    ...
    if (sprite.inheritanceInfo) {
      ...
      exemplar = this.project.sprites[sprite.inheritanceInfo.exemplar];
      if (exemplar) {
  ...
  model.sprites.childrenNamed("watcher").forEach((model) => {
    ...
    target = Object.prototype.hasOwnProperty.call(model.attributes, "scope")
      ? project.sprites[model.attributes.scope]
      : null;
    ...
    if (Object.prototype.hasOwnProperty.call(model.attributes, "var")) {
      watcher = new WatcherMorph(
        ...
        color,
        isNil(target) ? project.globalVariables : target.variables,
        model.attributes["var"],
        hidden
      );
    } else {
...
SnapSerializer.prototype.openProject = function (project, ide) {
  ...
  ide.siblings().forEach((morph) => morph.destroy());
...
StageMorph.prototype.toXML = function (serializer) {
  ...
  return serializer.format(
    ...
    ide && ide.globalVariables ? serializer.store(ide.globalVariables) : ""
  );
};
...
ProjectDialogMorph.prototype.buildContents = function () {
  ...
  if (this.task === "save") {
    ...
    this.classroomListField.typeInPadding =
    ...
    this.classroomListField.drawRectBorder =
...
ProjectDialogMorph.prototype.setSource = function (source) {
  ...
  if (this.source === "local") {
    ...
    this.listField.action = (item) => {
      ...
      src = this.ide.getURL(this.ide.resourceURL("Tools", item.fileName));
      xml = this.ide.serializer.parse(src);
      this.notesText.text = xml.childNamed("notes").contents || "";
      this.notesText.rerender();
      this.notesField.contents.adjustBounds();
...
ProjectDialogMorph.prototype.fixClassRoomItemColors = function () {
  ...
  this.classroomListField.contents.children[0].children.forEach(function (
    item
  ) {
